...
namespace Demo.FrameWork
{
    public class Utility
    {
        ...
        public const int c_hideLayer = 31;      // 不显示的layer
        public const int c_UIBottomBg = 18;     // UI 底部背景
        ...
        {
            GameObject content = XSingleton<CResourceManager>.GetInstance().GetResource(prefebPath, typeof(GameObject), enResourceType.UISprite, needCached, unloadBelongedAssetBundleAfterLoaded).m_content as GameObject;
            if (content == null)
            {
                content = XSingleton<CResourceManager>.GetInstance().GetResource(s_Sprite_Dynamic_Icon_Dir + "0", typeof(GameObject), enResourceType.UISprite, true, true).m_content as GameObject;
            }
            return content;
        }
                ...
                else
                {
                    ...
                    if (component != null)
                    {
                        ...
                        if ((isShowSpecMatrial && (component.sharedMaterial != null)) && ((component.sharedMaterial.shader != null) && !component.sharedMaterial.shader.name.Equals(s_ui_defaultShaderName)))
                        {
                            ...
                            image.set_material(component.sharedMaterial);
